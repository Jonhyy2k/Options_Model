...
def price(
    ...
    for n in range(n_terms):
        ...
        r_n = r - lam * k_comp + n * (mu_j + 0.5 * sig_j**2) / T

        d1 = (np.log(S / K) + (r_n + 0.5 * sigma_n**2) * T) / (sigma_n * np.sqrt(T))
        d2 = d1 - sigma_n * np.sqrt(T)

        if right == "C":
            bsm_n = S * norm.cdf(d1) - K * np.exp(-r_n * T) * norm.cdf(d2)
        else:
            bsm_n = K * np.exp(-r_n * T) * norm.cdf(-d2) - S * norm.cdf(-d1)
...
def price_for_calibration(
    ...
    for right_val in ("C", "P"):
        ...
        if mask.any():
            prices[mask] = price(
                S, K[mask], T, r, sigma, lam, mu_j, sig_j, right=right_val
            )
    return prices
