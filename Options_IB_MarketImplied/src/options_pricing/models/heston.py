...
def price(
    ...
    cf1 = _characteristic_function(phi, S, T, r, q, v0, kappa, theta, xi, rho, j=1)
    cf2 = _characteristic_function(phi, S, T, r, q, v0, kappa, theta, xi, rho, j=2)
    ...
    if right == "C":
        ...
        prices = call - S * np.exp(-q * T) + K * np.exp(-r * T)
...
def price_for_calibration(
    ...
    for right_val in ("C", "P"):
        ...
        if mask.any():
            prices[mask] = price(
                S, K[mask], T, r, q, v0, kappa, theta, xi, rho, right=right_val
            )
    return prices
