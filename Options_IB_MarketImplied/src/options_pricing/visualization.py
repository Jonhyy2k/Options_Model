...
def plot_density(
    ...
    if forward is not None:
        ax.axvline(forward, color=_ACCENT_PINK, linestyle="-.", linewidth=1.5,
                   label=f"Forward: ${forward:.2f}", zorder=4)
    ...
    if weights:
        ...
        market_lines = []
        if carry_source:
            market_lines.append(f"Carry: {carry_source}")
        if implied_rate is not None:
            market_lines.append(f"r: {implied_rate:.2%}")
        if dividend_yield is not None:
            market_lines.append(f"q: {dividend_yield:.2%}")
        if forward is not None:
            market_lines.append(f"F(0,T): ${forward:.2f}")
        if market_lines:
...
def plot_vol_surface_3d(
    ...
    for i, dte in enumerate(dte_grid):
        ...
        for j, K in enumerate(strike_grid):
            ...
            for name, res in cal_results.items():
                ...
                try:
                    p = float(model.price_for_calibration(
                        params, S, np.array([K]), T, r, np.array(["C"]), q=q
                    )[0])
            ...
            if bma_price <= intrinsic + 0.001 or bma_price >= S * np.exp(-q * T):
                continue
            try:
                def _obj(sig, _K=K, _T=T, _price=bma_price):
                    return float(bsm_mod.price(S, _K, _T, r, sig, q=q, right="C")) - _price
...
def plot_vol_smile(
    ...
    for name in sorted(cal_results.keys()):
        ...
        for k, mp in zip(calls["strike"], model_prices):
            try:
                intrinsic = max(S * np.exp(-q * T) - float(k) * np.exp(-r * T), 0)
                if mp <= intrinsic + 0.001 or mp >= S * np.exp(-q * T):
                    model_iv.append(np.nan)
                    continue
                def _obj(sig, _k=k, _mp=mp):
                    return float(bsm.price(S, _k, T, r, sig, q=q, right="C")) - _mp
                f_lo, f_hi = _obj(0.001), _obj(3.0)
                if f_lo * f_hi < 0:
...
def generate_all(
    ...
    plot_density(
        rnd,
        snapshot.spot,
        snapshot.forward_price,
        target,
        ticker,
        snapshot.expiry,
        snapshot.dte,
        snapshot.carry_source,
        snapshot.risk_free_rate,
        snapshot.dividend_yield,
        avg.weights,
        str(out / f"{ticker}_pdf.png"),
    )
