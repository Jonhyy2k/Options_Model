...
from options_pricing.market_inputs import MarketImpliedInputs, infer_market_inputs
...
class OptionsSnapshot:
    ...
    dte: int
    risk_free_rate: float      # rate used by the pricing models
    ...
    carry_source: str = "flat_rate"
    market_inputs: MarketImpliedInputs | None = None
...
def _apply_market_inputs(
    snapshot: OptionsSnapshot,
    *,
    carry_mode: str,
) -> OptionsSnapshot:
    """Populate implied carry inputs on a snapshot."""
    use_market_implied = carry_mode == "market_implied"
    market_inputs = infer_market_inputs(
        snapshot.chains,
        snapshot.spot,
        snapshot.dte / 365.0,
        fallback_rate=float(snapshot.input_rate or snapshot.risk_free_rate),
        use_market_implied=use_market_implied,
    )
    snapshot.risk_free_rate = market_inputs.implied_rate
    snapshot.dividend_yield = market_inputs.dividend_yield
    snapshot.discount_factor = market_inputs.discount_factor
    snapshot.forward_price = market_inputs.forward_price
    snapshot.carry_source = market_inputs.source
    snapshot.market_inputs = market_inputs
    return snapshot
...
def fetch_options_chain(
    ...
    snapshot = OptionsSnapshot(
        ...
        chains=df,
        input_rate=risk_free_rate,
    )
    return _apply_market_inputs(snapshot, carry_mode=carry_mode)
...
def save_snapshot(snapshot: OptionsSnapshot, path: str) -> None:
    ...
    meta = {
        ...
        "discount_factor": snapshot.discount_factor,
        "forward_price": snapshot.forward_price,
        "carry_source": snapshot.carry_source,
    }
...
def load_snapshot(
    ...
    *,
    ...
    snapshot = OptionsSnapshot(
        ...
        chains=df,
        input_rate=float(meta.get("input_rate", meta["risk_free_rate"])),
        dividend_yield=float(meta.get("dividend_yield", 0.0) or 0.0),
        discount_factor=float(meta["discount_factor"]) if meta.get("discount_factor") is not None else None,
        forward_price=float(meta["forward_price"]) if meta.get("forward_price") is not None else None,
        carry_source=str(meta.get("carry_source", "flat_rate")),
    )
    return _apply_market_inputs(snapshot, carry_mode=carry_mode)
