"""CLI entry point for options-pricing (Bloomberg version)."""
...
def _fetch_or_load(args) -> "OptionsSnapshot":
    """Get snapshot from Bloomberg or from a CSV file."""
...
def _add_common_args(p: argparse.ArgumentParser) -> None:
    ...
    p.add_argument("--ticker", default="SPY", help="Underlying symbol (e.g., AAPL, SPY)")
    ...
    p.add_argument("--from-csv", type=str, default=None,
                   help="Load snapshot from CSV instead of fetching from Bloomberg")
...
def main(argv: list[str] | None = None) -> None:
    parser = argparse.ArgumentParser(
        description="OptiQ — Options pricing with Bayesian Model Averaging (Bloomberg)"
    )
    sub = parser.add_subparsers(dest="command")

    # --- fetch command (Stage 1) ---
    fetch_p = sub.add_parser("fetch", help="Fetch options chain from Bloomberg and display/save")
    ...
    if args.command == "fetch":
        ...
        if args.dte_targets:
            if hasattr(args, "from_csv") and args.from_csv:
                ...
                print("       Use live Bloomberg data for multi-horizon analysis.")
                sys.exit(1)
